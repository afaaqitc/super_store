...
dd<-clean_names(dd)
dd<-dd %>% mutate(order_date=mdy(order_date),ship_date=mdy(ship_date)) 
...
mutate(ship_period=ship_date-order_date,days=weekdays(order_date),profits=as.numeric(profit),qty=as.numeric(quantity),discount=as.numeric(discount))  


#----------------------------------------------------------------------

daily_sales<-
dd %>% group_by(order_date) %>% summarize(orders=n(),quantites=sum(quantity),
sales=sum(sales),profits=sum(profit)) %>% arrange(order_date)
...
ship_modes_chart<-
dd %>% group_by(ship_mode) %>% summarize(sales=comma(sum(sales))) %>% ggplot(aes(x="",y=sales,fill =ship_mode))+geom_bar(stat = "identity")+coord_polar(theta = "y")+theme_void()+geom_text(aes(label =paste( round(sales/1000,digits =0),"K",sep = "")),position = position_stack(vjust = 0.5),color="white")+scale_fill_brewer(palette = "Set1")
...
sales=sum(sales),profits=sum(profits)) 


segment_totals_chart<-
segment_totals %>%mutate(years=factor(years)) %>%
ggplot(aes(x=years,y=sales,fill=segment))+
geom_bar(stat = "identity")+
geom_text(aes(label =comma(round(sales),digits = 0)),size=4,color="white",position = position_stack(vjust = 0.7),fontface = "bold")+
scale_y_continuous(labels =  comma)+
theme_bw()+labs(x="",y="")+
theme(axis.text.x = element_text(face = "bold",size = 10,colour = "black"),
      axis.text.y = element_text(face = "bold",size = 10,colour = "black"))+scale_fill_brewer(palette = "Dark2")+facet_wrap(~segment)
...
arrange(desc(sales)) %>% mutate(class=ifelse(sales>100000,"A",ifelse( sales>50000,"B","D")))%>% 
select(class,state,orders,quantities,sales,profits) %>% rename_with(.cols =everything() ,.fn = toupper)

state_sales_dt<-
state_sales %>% datatable(class="hover row-border",extensions = list(Buttons=TRUE,RowGroup=TRUE),
options = list(pageLength=15,dom="Bfrtip",buttons=c("csv","excel","pdf"),rowGroup=list(dataSrc=c(1)),
columnDefs=list(list(className="dt-center",targets=c(1:6)),list(visible=FALSE,targets=c(1))))) %>% 
formatCurrency(columns =c(5,6),digits = 2)

state_sales_top_chart<-
state_sales %>% filter(CLASS=="A")%>% 
ggplot(aes(x=reorder(STATE,SALES),y=SALES,fill=STATE))+geom_bar(stat = "identity")+labs(x="",y="")+theme_bw()+
geom_text(aes(label =comma(round(SALES),digits = 0)),size=4,color="white",position = position_stack(vjust = 0.6),fontface = "bold")+
  scale_fill_brewer(palette = "Set1")+scale_y_continuous(labels = comma)+
theme(axis.text.x = element_text(face = "bold",colour = "black",size = 10),axis.text.y = element_text(face = "bold",colour = "black",size = 10))
...
#---------------------------------------------------------------------------------------------------
category_orders<-
dd %>% group_by(category,sub_category) %>% 
...
dd %>% mutate(years=factor(years))%>% group_by(category,years) %>% 
summarize(sales=sum(sales),.groups = "drop") %>%
arrange( desc(sales)) %>% 
ggplot(aes(x=category,y=sales,fill=years))+geom_bar(stat = "identity",position = "dodge")+
...
category_orders_dt<-
dd %>% group_by(category,years) %>% 
summarize(sales=round(sum(sales)),.groups = "drop") %>%
arrange( desc(sales)) %>%  
pivot_wider(id_cols =category ,names_from = years,values_from = sales,values_fill = FALSE) %>% 
select(category,"2014","2015","2016","2017")%>%adorn_totals("col") %>% 
...
dd %>% group_by(category) %>% summarize(sales=sum(sales),.groups = "drop") %>% 
ggplot(aes(x="",y=sales,fill=category))+geom_bar(stat = "identity")+coord_polar(theta = "y")+
theme_void()+geom_text(aes(label = comma(sales)),size=5,color="white",fontface = "bold",position = position_stack(vjust = 0.5))

items_classes_top<- 
category_orders %>% select(sub_category,class) %>% 
left_join(dd,by="sub_category") %>% 
  group_by(class,category.x,sub_category,product_name) %>% summarize(quantities=sum(qty),
sales=sum(sales),.groups = "drop") %>%  filter(class %in% c("A","B")) 

items_classes_top_dt<-
items_classes_top %>%mutate(product_name=toupper(product_name)) %>%filter(class=="A") %>% select(category.x,sub_category,product_name,quantities,sales) %>% rename_with(.cols = c(1:5),.fn = toupper)%>%   
datatable(class="hover row-border",extensions = list(Buttons=TRUE,RowGroup=TRUE),
options = list(pageLength=15,dom="Bfrtip",buttons=c("csv","excel","pdf"),rowGroup=list(dataSrc=c(1,2)),
columnDefs=list(list(className="dt-center",targets=c(1,2,4,5)),list(visible=FALSE,targets=c(1,2))))) %>% 
...
columnDefs=list(list(className="dt-center",targets=c(1:10)),list(visible=FALSE,targets=1)))) %>% formatCurrency(columns = c(3:10),digits = 2)


ym_sales_chart<-
dd %>% mutate(years=factor(years),quarters=quarters(order_date))%>%  group_by(years,quarters) %>% summarize(sales=sum(sales)) %>%
    ggplot(aes(x=years,y=sales,fill=quarters))+geom_bar(stat = "identity")+geom_text(aes(label=comma(round(sales,digits =0),digits = 0)),fontface = "bold",position = position_stack(vjust = 0.5),size=4,color="white")+
    theme_classic()+
...
detect_model_chart<-
detect_model %>% select(date,sales,forecast) %>% 
pivot_longer(!date,names_to ="cat" ,values_to ="values" ) %>% 
mutate(date=as.POSIXct(date)) %>%  
ggplot(aes(x=date,y=values,color=cat,group=cat))+geom_line(size=1.2)+
...
wk_rts<-ts(weekly_sales$sales,start = c(2014,1),end = c(2017,12),frequency = 52)
...
element_text(face = "bold",colour = "black",size = 9),
axis.text.y = element_text(face = "bold",colour = "black",size = 9))+
scale_fill_brewer(palette = "Set1")+
scale_x_datetime(date_breaks ="2 week" ,date_labels = "%b%Y")+
labs(x="",y="")+scale_x_datetime(date_breaks = "4 week",date_labels = "%V-%b")+
scale_y_continuous(labels = comma)
...
### Year_month
```{r}
date_map_sales_dt %>%formatCurrency(columns = c(3:9),digits = 2)
```


### Chart
```{r}
ggplotly( ym_sales_chart,width=1200)
...
### Chart 
```{r}
ggplotly( segment_totals_chart,width=1300)
```
...
### Top States
```{r}
ggplotly( state_sales_top_chart,width=600)
```
