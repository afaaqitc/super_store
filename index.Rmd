...
dd<-read.csv("~/Data/Learn/R_Projects/store/data_store.csv")
...
select(class,states,orders,quantities,sales,profits) %>% rename_with(.cols =everything() ,.fn = toupper)
...
ggplot(aes(x=reorder(STATES,SALES),y=SALES,fill=STATES))+geom_bar(stat = "identity")+labs(x="",y="")+theme_bw()+
geom_text(aes(label =comma(round(SALES))),size=3,color="white",position = position_stack(vjust = 0.6))+
...
items_classes_top %>%mutate(product_name=toupper(product_name)) %>%filter(class=="A") %>% select(categories.x,sub_categories,product_name,quantities,sales) %>% rename_with(.cols = c(1:5),.fn = toupper)%>%   
datatable(class="hover row-border",extensions = list(Buttons=TRUE,RowGroup=TRUE),
...
### Top States
```{r}
ggplotly( state_sales_top_chart,width=700)
```
